function CGame(oData) {
    ...
    this._init = function () {
        ...
        //添加背景top
        var topbgSprite = s_oSpriteLibrary.getSprite('topbg');
        topbg = createBitmap(topbgSprite);
        topbg.y = 0;
        s_oStage.addChildAt(topbg);
        ...
        s_oStage.setChildIndex(topbg, BG_GAME3_INDEX);
        // //加分
        // text_add_score = new createjs.Text("+" + SCORE_BONUS, "80px gotham_boldregular", "#ff6527");
        // text_add_score.y = 35;
        // text_add_score.x = 230;
        // text_add_score.scaleX=0;
        // text_add_score.scaleY=0;
        // s_oStage.addChild(text_add_score);
    };
    ...
    this._createRow = function (iY) {
        ...
        var _iCurAreaRange = (CANVAS_WIDTH / iNumObstacles);
        ...
        if (!_bGameOver && !_bBonusTaken) {
            ...
            createjs.Tween.get(oSprite).to({
                ...
                createjs.Tween.get(oSprite).to({
                    ...
                    // //加分动画
                    // createjs.Tween.get(text_add_score).to({
                    //     scaleX: 1,
                    //     scaleY: 1
    ...
    this.setScore = function (iScoreToAdd) {
        ...
        if (iValue >= 0 && _iSpeedToAdd <= SPEED_INCREASE * 7) {
            ...
            oSprite.regY = 106 / 2;
            s_oStage.addChild(oSprite);
            createjs.Tween.get(oSprite).to({x: CANVAS_WIDTH / 2}, 1000).wait(100).call(function () {
    ...
    this.gameOver = function () {
        ...
        if (!_bGameOver) {
            var failNum = Math.round(Math.random()*5+1);
    ...
    this.update = function () {
        if (_bUpdate === false) {
            ...
            //控制topbg的移动
            topbg.y = topbg.y - OBSTACLES_SPD;
            if ((0 - topbg.y) > 462) {
                s_oStage.removeChild(topbg);
            }
